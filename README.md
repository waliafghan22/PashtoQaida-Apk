<div align="center">

# پښتو قاعده
# Pashto Qaida

### 📚 Learn Pashto Alphabet - For Kids Age 4-10

[![Kotlin](https://img.shields.io/badge/Kotlin-2.3.10-7F52FF?logo=kotlin&logoColor=white)](https://kotlinlang.org)
[![Compose Multiplatform](https://img.shields.io/badge/Compose%20Multiplatform-1.7.3-4285F4?logo=jetpackcompose&logoColor=white)](https://www.jetbrains.com/lp/compose-multiplatform/)
[![Platform](https://img.shields.io/badge/Platform-Android%20|%20Desktop%20|%20iOS-green)]()

<img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/icons/pashto-qaida-icon.webp" alt="Pashto Qaida Logo" width="150"/>

**A beautiful, interactive app to teach children the Pashto alphabet**

[Features](#-features) • [Screenshots](#-screenshots) • [Download](#-download) • [Tech Stack](#-tech-stack) • [Getting Started](#-getting-started) • [Contributing](#-contributing)

---

</div>

## 🌟 About

**Pashto Qaida** is a free, open-source educational app designed to help children aged 4-10 learn the Pashto alphabet. With colorful visuals, native audio pronunciation, interactive writing practice, and fun quizzes, learning Pashto has never been more engaging!

### Why This App?

- 🌍 **Preserve Pashto Language** - Help the next generation connect with their heritage
- 👨‍👩‍👧‍👦 **For Diaspora Families** - Overseas Pashtun families can teach kids their mother tongue
- 🏫 **Schools & Madrassas** - Perfect for educational institutions
- 📱 **Modern Learning** - Interactive, gamified approach kids love
- 🆓 **Free & Open Source** - Available to everyone, forever

---

## ✨ Features

### 📚 Learn Letters
- All **44 Pashto letters** with proper pronunciation
- Native speaker audio for each letter
- Example words with colorful illustrations
- Right-to-Left (RTL) layout support

### ✍️ Writing Practice
- Interactive letter tracing
- Visual guides for proper stroke order
- Finger/stylus drawing on canvas
- Undo, clear, and complete actions

### 🎮 Fun Quizzes
- **Letter Match** - Match letters to pictures
- **Sound Match** - Hear sound, pick the letter
- **Word Match** - Match words to starting letters
- Progressive difficulty based on learned letters

### 🏆 Progress & Rewards
- ⭐ Earn stars for completing activities
- 🏅 Unlock achievement badges
- 📊 Track learning progress
- 🔓 Unlock new letters as you learn

### 👨‍👩‍👧 Parent Features
- 🔒 PIN-protected parent area
- 📈 View child's progress statistics
- 🔄 Reset progress option
- 🔇 Sound on/off toggle

### 🌐 Multilingual
- Full **Pashto** interface
- Full **English** interface
- Easy language switching

### 📴 Works Offline
- Download once, use anywhere
- No internet required after installation
- All audio and images bundled

---

## 📸 Screenshots

<div align="center">

| Home Screen | Letters Grid | Letter Detail |
|:-----------:|:------------:|:-------------:|
| <img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/pashto-qaida-1.webp?raw=true" width="250"/> | <img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/pashto-qaida-2.webp" width="250"/> | <img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/pashto-qaida-3.webp" width="250"/> |

| Writing Practice | Quiz Game | Progress |
|:----------------:|:---------:|:--------:|
| <img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/pashto-qaida-4.webp" width="250"/> | <img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/pashto-qaida-5.webp" width="250"/> | <img src="https://github.com/waliafghan22/waliafghan22.github.io/blob/main/images/projects/pashto-qaida-6.webp" width="250"/> |

</div>


---

## 🛠️ Tech Stack

| Component | Technology |
|-----------|------------|
| **Framework** | [Compose Multiplatform](https://www.jetbrains.com/lp/compose-multiplatform/) 1.7.3+ |
| **Language** | [Kotlin](https://kotlinlang.org/) 2.3.10+ |
| **Database** | [SQLDelight](https://cashapp.github.io/sqldelight/) 2.2.0+ |
| **DI** | [Koin](https://insert-koin.io/) 4.1.1+ |
| **Navigation** | Navigation 3 + Adaptive Layouts |
| **ViewModel** | Lifecycle ViewModel (Official) |
| **Animations** | [Compottie](https://github.com/nicholasdille/compottie) (Lottie) |
| **Testing** | kotlin.test + [Turbine](https://github.com/cashapp/turbine) |

### Architecture
- **MVVM** (Model-View-ViewModel)
- **Clean Architecture** principles
- **Unidirectional Data Flow**
- **Repository Pattern**

---


📬 Contact
Developer: [Mohammed Wali]

📧 Email: waliafghan22@gmail.com
🐦 Twitter: @waliafghan22
💼 LinkedIn: waliafghan22
🌐 Website: https://waliafghan22.github.io
Project Links
🏠 Homepage
🐛 Issue Tracker
📖 Documentation
📥 Releases
⭐ Support
If you find this app helpful:

⭐ Star this repository
📢 Share with Pashtun families
💬 Feedback is always welcome!
<div align="center">
💚 Made with love for Pashtun children worldwide
پښتو ژبه دی ژوندۍ وي!

May the Pashto language live forever!

<img src="screenshots/footer_banner.png" alt="Pashto Qaida" width="600"/></div> ```
